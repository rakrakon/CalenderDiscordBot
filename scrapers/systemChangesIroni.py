...
class ChangesSpider(scrapy.Spider):
    ...
    def parse(self, response):
        data = {
            '__EVENTTARGET': 'dnn$ctr3413$TimeTableView$btnChanges',
            'dnn$ctr3413$TimeTableView$ClassesList': '21',
            'dnn$ctr3413$TimeTableView$ControlId': '2',
        }
    ...
    def parse_table(self, response):
        ...
        try:
            df2 = pd.read_csv('data\changesIroni.csv')
        except Exception:
            df2 = pd.DataFrame()
        ...
        for i,df in enumerate(dfs):
            if i == 11:
                ...
                try:
                    ...
                    df_diff.to_csv('data\changesDiffIroni.csv', index=False)
                    print(df_diff)
                except Exception:
                    print(f'df_diff: {df_diff}')
                outputDF.to_csv('data\changesIroni.csv', index=False) 